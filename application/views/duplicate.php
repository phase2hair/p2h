...
<head>
    ...
    <section class="content" style="background-image: url(<?=$bg1?>); background-repeat: no-repeat;background-size: cover;background-position: center;">
      ...
      <div class="box">
        <div class="box-header with-border">
          <h3 class="box-title"><i class='fa fa-users fa-lg'></i> Possible duplicate entries</h3>
        </div>
        <div class="box-body">
        <?php if(count($dupe)!==0){ ?>
                <tr>
  ...
  Copyright &copy; The EasyPC Company <b>Lee Fowler</b> <?=Date('Y')?> [Duplicate]
  </footer>
...
<script>
  ...
  function delete_user(del_link, client){
      var r = confirm("Are you sure to delete "+client+"?");
        if (r == true) {
            location.href=del_link+'/?url=<?=uri_string()?>';
        } else {
            return false;
        }
  }
